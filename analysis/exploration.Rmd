...
There are `dat %>% select(range, column) %>% distinct()` grid cells in total, defined by `range` and `column`.
...
```{r, eval=FALSE, echo=FALSE}
save(dat_nest, fits, plot_fn, rngs, file = "output/traj_data.Rdata")
```
...
plot_fn(filter(dat_nest, cultivar == "PI152651")$data)
```
